...
function KPI({ title, value, subtitle, icon: Icon }: {title:string; value:string; subtitle?:string; icon:any}) {
  return <div className="card kpi"><div><div className="muted small">{title}</div><div className="kpi-value">{value}</div>{subtitle ? <div className="muted xs">{subtitle}</div> : null}</div><div className="icon-box"><Icon size={18} /></div></div>;
}
