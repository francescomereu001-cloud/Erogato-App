...
useEffect(() => {
  ...
  const availablePeriodOptions = useMemo(() => timeSeriesData.filter((row) => row.pratiche > 0 || row.erogato > 0), [timeSeriesData]);
  ...
  useEffect(() => {
    const selectablePeriods = availablePeriodOptions.length ? availablePeriodOptions : timeSeriesData;
    if (!selectablePeriods.length) {
      setSelectedPeriodKey('');
      return;
    }

    const hasCurrent = selectablePeriods.some((row) => {
      const candidateKey = viewGranularity === 'monthly' ? String(row.monthIndex) : row.key;
      return candidateKey === selectedPeriodKey;
    });

    if (!hasCurrent) {
      const latest = selectablePeriods[selectablePeriods.length - 1];
      setSelectedPeriodKey(viewGranularity === 'monthly' ? String(latest.monthIndex) : latest.key);
    }
  }, [availablePeriodOptions, timeSeriesData, selectedPeriodKey, viewGranularity]);
 ...
 async function handleFiles(fileList: FileList | null) {
  ...
  try {
    ...
    const dedupedImportedRows = Array.from(new Map(importedRows.map((row) => [row.stableIdentity, row])).values());
    ...
    const payload = Array.from(new Map(dedupedImportedRows.map((row) => [row.stableIdentity, {
      unique_key: row.stableIdentity,
      data_liquidazione: row.dateISO
        ? new Date(row.dateISO).toISOString().slice(0, 10)
        : null,
      importo_finanziato: Number.isFinite(row.importoFinanziato) ? row.importoFinanziato : 0,
      prodotto: Number.isFinite(Number(row.prodottoCode)) ? Number(row.prodottoCode) : null,
      dealer: row.dealer || null,
      subagente: row.subagente || null,
      provvigione: Number.isFinite(row.provvigione) ? row.provvigione : 0,
      polizza: Number.isFinite(row.polizza) ? row.polizza : 0,
      cliente: row.cliente || null,
      codice_fiscale: row.codiceFiscale || null,
      tabella: row.tabella || null,
      numero_rate: Number.isFinite(row.numeroRate) ? row.numeroRate : null,
      importo_rata: Number.isFinite(row.importoRata) ? row.importoRata : null,
    ...
    if (error) {
      ...
      alert(`Errore nel salvataggio su Supabase${error.message ? `: ${error.message}` : ''}`);
      setDataSourceMode(rows.length ? 'supabase' : 'local');
    }
  } finally {
    setUploading(false);
  }
            ...
            <div className="panel">
              ...
              <div className="filters-grid period-grid">
                <select className="select" value={selectedPeriodKey} onChange={(e) => setSelectedPeriodKey(e.target.value)}>
                  {(availablePeriodOptions.length ? availablePeriodOptions : timeSeriesData).map((item) => (
