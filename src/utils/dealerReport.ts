...
export function buildDealerReportHtml(data: DealerReportData) {
  ...
  const ytdProvCurrent = data.ytdCurrentRows.reduce((s, r) => s + r.provvigione, 0);
  const ytdProvPrev = data.ytdPrevRows.reduce((s, r) => s + r.provvigione, 0);
  ...
  <section class="page"><div class="hero"><div class="eyebrow">Report commerciale dealer</div><div class="title">Sintesi commerciale</div><div class="dealer">${escapeHtml(data.dealerName)}</div><div class="meta"><div class="chip"><b>Dati aggiornati</b>${data.updatedAt.toLocaleDateString('it-IT')}</div><div class="chip"><b>Periodo YTD</b>Gen-${MONTHS_IT[data.ytdMonthLimit - 1]} ${data.currentYearValue}</div><div class="chip"><b>Stato</b>${escapeHtml(data.statoDealer)}</div><div class="chip"><b>Continuità</b>${escapeHtml(data.continuityLabel)}</div><div class="chip"><b>Azione</b>${escapeHtml(data.suggestedAction)}</div><div class="chip"><b>Tipo dealer</b>${escapeHtml(data.dealerType)}</div><div class="chip"><b>Copertura rate</b>${coverage === null ? 'n/d' : fmtPct(coverage)}</div></div></div>
  <div class="grid"><div class="card"><div class="label">Erogato YTD</div><div class="value">${euro0(ytdErogatoCurrent)}</div></div><div class="card"><div class="label">Erogato ultimi 12 mesi</div><div class="value">${euro0(data.sum(data.last12Rows))}</div></div><div class="card"><div class="label">Pratiche YTD</div><div class="value">${num(ytdPraticheCurrent)}</div></div><div class="card"><div class="label">Ticket medio YTD</div><div class="value">${euro0(ytdTicketCurrent)}</div></div><div class="card"><div class="label">Rate medie YTD</div><div class="value">${fmtRates(ytdRatesCurrent)}</div><div class="sub">${coverage === null ? 'Numero rate non disponibile' : `Disponibile sul ${fmtPct(coverage)} delle pratiche YTD`}</div></div><div class="card"><div class="label">Provvigioni YTD</div><div class="value">${euro0(ytdProvCurrent)}</div></div><div class="card"><div class="label">Polizze YTD</div><div class="value">${num(ytdPolizzeCurrent)}</div></div></div>
  <h2>Confronto YTD anno corrente vs stesso periodo anno precedente</h2><table><thead><tr><th>KPI</th><th class="right">${data.currentYearValue}</th><th class="right">${data.prevYearValue}</th><th class="right">Var. assoluta</th><th class="right">Var. %</th></tr></thead><tbody>${comparisonRow('Erogato', ytdErogatoCurrent, ytdErogatoPrev, euro0)}${comparisonRow('Pratiche', ytdPraticheCurrent, ytdPratichePrev, (v) => num(v))}${comparisonRow('Ticket medio', ytdTicketCurrent, ytdTicketPrev, euro0)}${comparisonRow('Rate medie', ytdRatesCurrent, ytdRatesPrev, (v) => num(v, 1))}${comparisonRow('Provvigioni', ytdProvCurrent, ytdProvPrev, euro0)}${comparisonRow('Polizze', ytdPolizzeCurrent, ytdPolizzePrev, (v) => num(v))}</tbody></table>
  ...
  <h2>Andamento erogato ultimi 12 mesi</h2>${bars}<h2>Insight commerciali automatici</h2><ul>${insightList}</ul><div class="footer">${escapeHtml(footer)}</div></section>
