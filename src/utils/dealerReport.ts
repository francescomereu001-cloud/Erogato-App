...
export function buildDealerReportHtml(data: DealerReportData) {
  ...
  <section class="page"><div class="hero"><div class="eyebrow">Report commerciale dealer</div><div class="title">Sintesi commerciale</div><div class="dealer">${escapeHtml(data.dealerName)}</div><div class="meta"><div class="chip"><b>Dati aggiornati</b>${data.updatedAt.toLocaleDateString('it-IT')}</div><div class="chip"><b>Periodo YTD</b>Gen-${MONTHS_IT[data.ytdMonthLimit - 1]} ${data.currentYearValue}</div><div class="chip"><b>Fonte</b>${escapeHtml(data.sourceLabel)}</div><div class="chip"><b>Filtri</b>${escapeHtml(data.contextSummary)}</div><div class="chip"><b>Stato</b>${escapeHtml(data.statoDealer)}</div><div class="chip"><b>Continuità</b>${escapeHtml(data.continuityLabel)}</div><div class="chip"><b>Azione</b>${escapeHtml(data.suggestedAction)}</div><div class="chip"><b>Tipo dealer</b>${escapeHtml(data.dealerType)}</div></div></div>
